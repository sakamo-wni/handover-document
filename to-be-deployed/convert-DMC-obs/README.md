...
## 概要
このLambda関数は、スペイン気象局（DMC: División Meteorológica de Cataluña）の気象観測データを処理します。S3からSQSを通じて受信したJSONデータを解析し、標準化されたフォーマットに変換してS3に保存します。データの効率的な処理のために、前回処理したデータとの差分比較を行い、変更があったデータのみを処理します。
